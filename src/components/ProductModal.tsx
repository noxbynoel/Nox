            ...
            <div>
                ...
                {product.ring_sizes && product.ring_sizes.length > 0 && (
                  <div className="mb-6">
                    <div className="flex justify-between items-center mb-2">
                      ...
                      <button onClick={() => setShowRingChart(true)} className="text-xs text-black dark:text-white underline hover:text-gray-500 transition-colors">Size Guide</button>
                    </div>
                    <div className="flex flex-wrap gap-2">
                      {product.ring_sizes.map(size => (
                        <button
                          ...
                          className={`px-4 py-2 border rounded-md text-sm font-medium transition-all duration-300 ${selectedSize === size ? 'border-black dark:border-white bg-black dark:bg-white text-white dark:text-[#121212]' : 'border-gray-200 dark:border-[#4A4A4A] hover:border-gray-300 dark:hover:border-gray-500 text-gray-900 dark:text-gray-100'}`}
                        >
                          {size}
                        </button>
                      ))}
              ...
              <div className="border-t border-gray-200 dark:border-[#4A4A4A] pt-6">
                ...
                <button
                  ...
                  className="w-full py-4 bg-black dark:bg-white text-white dark:text-[#363636] font-medium rounded-lg hover:bg-gray-800 dark:hover:bg-gray-300 transition-smooth disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center space-x-2"
                >
                  {adding ? (
      ...
      {showRingChart && (
        ...
        >
          <div className="relative bg-white dark:bg-[#1A1A1A] p-6 rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <h3 className="text-2xl font-serif font-bold text-black dark:text-white mb-6 text-center">Ring Size Guide</h3>
